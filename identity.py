...
async def ensure_current_riot_identity(discord_id: int):
    ...
    if not doc:
        return (False, "You need to link your Riot account first using `!linkriot Name#Tag`.", None)
    ...
    if not puuid and (not name or not tag):
        return (False, "Your Riot link looks incomplete. Re-link with `!linkriot Name#Tag`.", None)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if acc is None:
            return (False, "I couldn’t find your Riot account anymore. Re-link with `!linkriot Name#Tag`.", None)
